...
export function ButtonLink({ href, children, variant = "primary", dataCta, className }: ButtonLinkProps) {
  ...
  return (
    <Link
      ...
      onClick={handleClick}
      data-cta={dataCta}
      className={`cta-glow inline-flex items-center justify-center rounded-full px-6 py-3 text-sm font-bold transition-colors focus-visible:outline-none ${variants[variant]} ${className ?? ""}`.trim()}
    >
      {children}
    </Link>
  );
