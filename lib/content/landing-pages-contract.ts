...
export type LandingPagesContent = SiteContent["landingPages"];

type ValidationResult =
  | {
      ok: true;
      value: LandingPagesContent;
    }
  | {
      ok: false;
...
export function pickLandingPagesContent(content: SiteContent): LandingPagesContent {
  return content.landingPages;
}

export function mergeLandingPagesContent(current: SiteContent, landingPages: LandingPagesContent): SiteContent {
  return {
    ...current,
    landingPages
  };
...
export function validateAndSanitizeLandingPagesContent(input: unknown): ValidationResult {
  const errors: FieldErrors = {};
  const sanitized = validateAndSanitizeByTemplate(input, siteContent.landingPages, "content", errors) as LandingPagesContent;
